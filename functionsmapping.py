...
class OxideDataProcessor:
    def __init__(self, data, oxide_column_indices):
        """
        Initialize the processor with data and oxide column indices.

        :param data: numpy array representing the oxide data
        :param oxide_column_indices: dictionary mapping oxide names to their column indices
        """
        self.data = np.copy(data)
        self.oxide_column_indices = oxide_column_indices

    def clean_data(self):
        """Flag bad analyses or cracks based on FeO, CaO, and Total thresholds."""
        clean = np.copy(self.data)
        a, b = clean.shape
        
        feo_index = self.oxide_column_indices.get('FeO', None)
        cao_index = self.oxide_column_indices.get('CaO', None)
        total_index = self.oxide_column_indices.get('Total', None)
    ...
    def to_anhydrous(self):
        """Normalize oxide compositions to 100%."""
        anh = np.copy(self.data)
        total_index = self.oxide_column_indices.get('Total', None)
        a, b = anh.shape
        ...
        for i in range(a):
            for j in range(b):
                if anh[i, total_index] > 0:
                    anh[i, j] = anh[i, j] * 100 / anh[i, total_index]
        
        return anh

    def to_mol(self):
        """Convert oxide compositions to mol values."""
        mol = np.copy(self.data)
        mw = {
        ...
        for i in range(mol.shape[0]):
            for col_name, col_index in self.oxide_column_indices.items():
                if mol[i, total_index] > 0:
                    if col_name == "Total":
                        mol[i, col_index] = 100
                    else:
                        mol[i, col_index] = mol[i, col_index] / mw.get(col_name, 1)
        
        return mol

    def to_cat(self):
        """Convert mol oxide compositions to cation fractions."""
        ...
        for i in range(cat.shape[0]):
            if cat[i, total_index] > 0:
                for col_name, col_index in self.oxide_column_indices.items():
                    if col_name != "Total":
                        cat[i, col_index] = cat[i, col_index] * ct.get(col_name, 1)
                
                # Recalculate totals as cation fractions
                oxide_columns = [index for col_name, index in self.oxide_column_indices.items() if col_name != "Total"]
                total_cations = np.sum(cat[i, oxide_columns])
                cat[i, total_index] = total_cations
                
                for col_name, col_index in self.oxide_column_indices.items():
                    if col_name != "Total":
                        cat[i, col_index] /= total_cations
        
        return cat

    def add_fe2o3(self, fe2_fetot=0.7):
        """Calculate and add Fe2O3 column based on Fe2+/Fetot ratio."""
        updated_data = np.copy(self.data)
        
        if "Fe2O3" not in self.oxide_column_indices:
            feo_index = self.oxide_column_indices.get('FeO', None)
            fe2o3_values = updated_data[:, feo_index] * (1 - fe2_fetot) * 1.11134
            updated_data = np.append(updated_data, fe2o3_values[:, np.newaxis], axis=1)
            updated_data[:, feo_index] *= fe2_fetot
            self.oxide_column_indices["Fe2O3"] = updated_data.shape[1] - 1
        else:
            print("Fe2O3 column already exists.")
        
        return updated_data

    def norm_calc(self):
        """Perform normative calculations."""
        # Detailed implementation goes here. This function will follow the same
        # logic as the existing implementation but adapted for the class structure.
        pass
